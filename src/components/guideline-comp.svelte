<template lang="pug">
  ...
  +if("isShow")
    mask-comp(transparent notyping)
      +if("!isBusy")
        .dummy.dummy1(bind:this="{dummyElement1}" style="{currentDummyStyle}" transition:fade="{{duration: transition ? 300 : 0}}")
        .dummy.dummy2(bind:this="{dummyElement2}" style="{currentDummyStyle}" transition:fade="{{duration: transition ? 300 : 0}}")
      +if("!isBusy")
